          ...
          <div v-if="artistDone" class="flex">
            <div class="flex">
              ...
              <div class="col fit">
                ...
                <h2 v-if="progress.artists">{{songsSaved}}</h2>
              </div>
            </div>
          </div>
          <Loading v-else/>
...
export default {
    ...
    methods: {
      ...
      async songAnalysis(id) {
        ...
        trackData = await this.songAnalysisFeatures(trackData, id);
        trackData.audioAnalysis = await this.cleanAudioAnalysis(id);
        return trackData;
      },
      // {id: String}
      async songAnalysisFeatures(trackData, id) {
          let song = trackData;
          let audioFeatures = await this.$store.dispatch('getAudioFeaturesForTrack', id);
      ...
      async cleanAudioAnalysis(id) {
          let audioAnalysisSegments = 80;
          let audioAnalysis = await this.$store.dispatch('getAudioAnalysisForTrack', id);
          let newSegments = [];
      ...
      async getPercentiles(payload) {
          let ids = Object.keys(this.$store.state.tracks);
          let keys = Object.keys(payload);
          let response = {valence: 0, danceability: 0, energy: 0, banger: 0};
          for (let i = 0; i < keys.length; i++) {
              let lower = 0;
              for (let j = 0; j < ids.length; j++) {
                  if (this.$store.state.tracks[ids[j]][keys[i]] <= payload[keys[i]]) {
                      lower += 1;
                  }
              }
              response[keys[i]] = lower / this.$store.state.progress.total;
          }
          return response;
      },
      //{h, s, v}
      ...
      async checkTracksLoaded() {
        if (this.progress.tracks) {
          ...
          this.trackData.percentiles = await this.getPercentiles({
    ...
    async created() {
      ...
      this.trackData = await this.songAnalysis(this.$route.params.id);
